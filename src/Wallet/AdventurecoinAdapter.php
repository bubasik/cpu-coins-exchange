...
use App\Api\AdventurecoinApi;
use App\Core\Config;

final class AdventurecoinAdapter implements CoinAdapter
{
    private HdWallet $wallet;

    public function __construct()
    {
        $this->wallet = new HdWallet(AdventurecoinApi::network());
    }

    public function symbol(): string { return 'ADVC'; }
    public function name(): string { return 'Adventurecoin'; }
    public function decimals(): int { return 8; }
    public function api(): \App\Api\ApiClient { return AdventurecoinApi::client(); }
    public function hdWallet(): HdWallet { return $this->wallet; }
    public function confirmationsRequired(): int { return AdventurecoinApi::confirmationsRequired(); }
    public function xpub(): string { return Config::get('ADVC_XPUB', ''); }
    public function xprv(): string { return Config::get('ADVC_XPRV', ''); }
    public function hotWalletAddress(): string { return Config::get('ADVC_HOT_ADDRESS', ''); }
    public function hdBasePath(): string { return Config::get('ADVC_HD_PATH', "m/44'/1234'/0'/0/"); }

    public function deriveDepositAddress(int $index): string
    {
        $xpub = $this->xpub();
        if (empty($xpub) || strpos($xpub, 'your-') !== false) {
            throw new \RuntimeException('ADVC_XPUB not configured. Run: php bin/generate-wallet.php adventurecoin');
        }
    ...
    public function buildPayoutTx(
        ...
        if (empty($xprv) || strpos($xprv, 'your-') !== false) {
            throw new \RuntimeException('ADVC_XPRV not configured');
        }
